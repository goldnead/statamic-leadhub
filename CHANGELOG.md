...
- **Outbound event surface.** The full set of `LeadHub*` lifecycle events is documented as a public integration point — pair it with [goldnead/statamic-webhook-manager](https://github.com/goldnead/statamic-webhook-manager) for CP-managed outbound webhooks, or use the built-in `webhook` CRM driver for a direct JSON POST.
