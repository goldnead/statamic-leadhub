...
function scheduledCommandCounts(): array
{
    ...
    foreach (app(Schedule::class)->events() as $event) {
        ...
        // "…/php 'artisan' leadhub:followups:due" → "leadhub:followups:due"
...
it('registers every scheduled command exactly once', function (): void {
    ...
    $counts = array_filter(
        ...
        fn (string $name): bool => str_starts_with($name, 'leadhub:'),
...
it('registers the scheduled commands at all', function (): void {
    // Guards the test above against passing for the wrong reason: an empty
    // schedule has no duplicates either.
    expect(scheduledCommandCounts())->toHaveKey('leadhub:followups:digest');
});
