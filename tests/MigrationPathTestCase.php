...
abstract class MigrationPathTestCase extends TestCase
{
    ...
    protected function isolatedConnection(): array
    {
        ...
        return [
            ...
            'database' => env('DB_DATABASE', 'leadhub_test').'_migration_path',
    ...
    {
        return sys_get_temp_dir().'/leadhub-migration-path-'.getmypid().'.sqlite';
    }

    /**
     * Empty database, brand-context installed, nothing of this addon's own.
     *
     * `brands` is a hard precondition for most of these tests: the brand-scoping
     * migration backfills every row onto the default brand and needs one to
     * exist. The test that checks what happens when it does *not* drops the
     * table again on purpose.
     */
    ...
    /**
     * Write a second contact for an address that already has one, and keep it.
     *
     * Only possible while no unique covers the column, which is the situation
     * this models — before 1.4.0 it was a plain index, so this is not a corrupt
     * install, it is the ordinary history of one.
     */
    protected function writeDuplicateContact(string $emailNormalized, array $overrides = []): int
    {
        return $this->copyContact($emailNormalized, $overrides);
    }

    /**
     * Whether a second contact can be written for an address that already has
     * one.
     *
     * This is the assertion the whole suite exists for, and it is deliberately
     * behavioural. "The migration ran" and "the constraint is there" are not
     * the same statement, and confusing the two is what let a release drop the
     * dedupe index without replacing it. An index by name can exist over the
     * wrong columns, over a nullable column, or not bite at all; the only thing
     * that settles it is trying to write the row the constraint is supposed to
     * refuse.
     */
    protected function duplicateContactIsAccepted(string $emailNormalized): bool
    {
        try {
        ...
        $this->isolated()->table('leadhub_contacts')->where('id', $id)->delete();

        return true;
    }

    /**
     * The same question for one of the other five brand-scoped identifiers.
     *
     * @param  array<string, mixed>  $overrides
     */
    protected function duplicateIsAccepted(string $table, string $column, string $value, array $overrides = []): bool
    {
        try {
    ...
    private function copyContact(string $emailNormalized, array $overrides = []): int
    {
        return $this->copyRow('leadhub_contacts', 'email_normalized', $emailNormalized, [
            'full_name' => 'Second record of the same person',
            ...$overrides,
        ]);
    }
     ...
     * @param  array<string, mixed>  $overrides
     */
    private function copyRow(string $table, string $column, string $value, array $overrides = []): int
    {
        $existing = $this->isolated()->table($table)->where($column, $value)->orderBy('id')->first();

        if (! $existing) {
            throw new \RuntimeException("No row in [{$table}] with {$column}=[{$value}] to duplicate.");
        }

        $row = collect((array) $existing)->except('id');

        if ($row->has('uuid')) {
            $row->put('uuid', (string) Str::uuid());
        }

        if ($row->has('dedupe_key') && $column !== 'dedupe_key') {
            $row->put('dedupe_key', (string) Str::uuid());
        }

        return (int) $this->isolated()->table($table)->insertGetId($row->merge($overrides)->all());
    }
}
