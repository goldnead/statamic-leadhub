...
const props = defineProps([
    'task',             // { contact_id, assignee_id, priority }
    'assignableUsers',
    'priorityOptions',
    'contactOptions',
    'contactSearchUrl',
...
function submit() {
    ...
    router.post(props.storeUrl, {
        ...
        // The CP DatePicker is reka-ui: its model is an
        // @internationalized/date object, never a string. Sending it raw is
        // what made follow-ups uncreatable in v1.4.
                ...
                <Card>
                    <div class="grid gap-4 sm:grid-cols-2 p-1">
                        <Field :label="__('Title')" :error="errors.title" class="sm:col-span-2">
                            <Input v-model="form.title" :placeholder="__('e.g. Call back about the quote')" data-leadhub-task-title />
                        </Field>
                        ...
                        <Field :label="__('Due')" :error="errors.due_at" class="sm:col-span-2">
                            <!-- granularity="minute", not `with-time` — that is
                                 not a prop of this component and silently does
                                 nothing (v1.5.0 changelog). -->
